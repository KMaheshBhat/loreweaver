...
import { BaseNode, kindSystemIntent } from '@engine/model'
import { kindWeaverIntentSubmitTurn } from '@engine/weaver/model'
...
app.whenReady().then(async () => {
  ...
  const l = engine.bootstrap() // l = mixin-less ledger
  ...
  ipcMain.handle('weaver:submit', async (_event, nodes: BaseNode[]): Promise<void> => {
    const submitIntent: Intent = {
      ...
      kind: kindWeaverIntentSubmitTurn,
      nodes: nodes,
      options: { source: 'client' }
    }
    console.log(`weaver:submit ${JSON.stringify(submitIntent)}`)
